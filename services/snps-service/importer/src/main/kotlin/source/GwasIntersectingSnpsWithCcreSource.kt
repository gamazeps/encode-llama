...
class GSGwasIntersectingSnpsWithCcreSource(private val gsParentPath: GSDirectory) : GwasIntersectingSnpsWithCcreSource {
    override fun import(sink: GwasIntersectingSnpsWithCcreSink) {
        log.info { "Beginning gwas Intersecting Snps GS File Import" }
        val allFiles = gsList(gsParentPath.bucket, gsParentPath.dirPath).filter {  !it.isDirectory && it.name.endsWith("_gwassnp_ccre.bed") }
        log.info { "allFiles size: ${allFiles.size}" }
        for (f in allFiles) {
            log.info { "Importing gwas Intersecting Snps With Ccre from file $f" }
...
class GwasIntersectingSnpsWithCcreFileSource(private val files: List<File>) : GwasIntersectingSnpsWithCcreSource {
    override fun import(sink: GwasIntersectingSnpsWithCcreSink) {
        log.info { "Beginning gwas Intersecting Snps With Ccre  File Import" }
        for (file in files) {
            log.info { "Importing gwas Intersecting Snps With Ccre  from file $file" }
