...
# Example usage: scripts/test-gateway.sh staging import_01_22_2019
set -e
...
# Select tag for peaks-service docker image
SERVICE_TAGS=( $(gcloud container images list-tags gcr.io/${GCR_PROJECT_ID}/peaks-service --limit=10 --format="get(tags)") )
echo "Please select peaks service docker image tag to deploy for test:"
select SERVICE_TAG in ${SERVICE_TAGS[@]}
do
    if [[ ! -z "${SERVICE_TAG}" ]]; then
        echo "Deploying test peaks-service ${SERVICE_TAG}..."
        break
    else
...
sed -e "s/\${SERVICE_VERSION}/${SERVICE_TAG}/" \
    -e "s/\${DB_SCHEMA}/${2}/" \
    -e "s/ peaks-service/ peaks-service-test"/ \
    k8s/service-${1}.yml | \
    kubectl apply -f -

# Wait for app test deployment to finish rolling out
kubectl rollout status deployment/peaks-service-test-deployment
...
kubectl delete service/peaks-service-test-service
kubectl delete deployment/peaks-service-test-deployment
