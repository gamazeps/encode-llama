...
if [[ ! -z "${3}" ]]; then
    ...
    TAGS=( $(gcloud container images list-tags gcr.io/devenv-215523/peaks-service --limit=10 --format="get(tags)") )
...
kubectl rollout status deployment/peaks-service-deployment
