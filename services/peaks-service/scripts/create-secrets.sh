...
# Create peaks-db-credentials secret if it does not exist
if ! kubectl get secret peaks-db-credentials &>/dev/null ; then
    echo "Creating peaks-db-credentials secret..."
    DB_USERNAME=$(jq -r ".peaks_db_username" /tmp/devops/staging-db-credentials/peaks-db.json)
    DB_PASSWORD=$(jq -r ".peaks_db_password" /tmp/devops/staging-db-credentials/peaks-db.json)
    kubectl create secret generic peaks-db-credentials \
        --from-literal=username=$DB_USERNAME \
        --from-literal=password=$DB_PASSWORD
else
    echo "Secret peaks-db-credentials already found. Skipping creation..."
fi
