...
fun requestEncodeSearch(encodeBaseUrl: String, species: String,searchTerm: String): EncodeSearchResult {
...
fun requestEncodeTarget(encodeBaseUrl: String, target: String): EncodeTarget {
    val experimentUrl = HttpUrl.parse("$encodeBaseUrl/targets/$target/")!!.newBuilder()
            .addQueryParameter("format", "json")
            .build()
    val experimentRequest = Request.Builder().url(experimentUrl).get().build()
    val experimentResultString = retry("Target Request", 3) {
        http.newCall(experimentRequest).execute().body()!!.string()
    }
    return moshi.adapter(EncodeTarget::class.java).fromJson(experimentResultString)!!
}
