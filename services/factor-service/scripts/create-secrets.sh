...
# Create factor-db-credentials secret if it does not exist
if ! kubectl get secret factor-db-credentials &>/dev/null ; then
    echo "Creating factor-db-credentials secret..."
    ...
    kubectl create secret generic factor-db-credentials \
        --from-literal=username=$DB_USERNAME \
        --from-literal=password=$DB_PASSWORD
else
    echo "Secret factor-db-credentials already found. Skipping creation..."
fi
