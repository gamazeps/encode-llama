...
if [[ ! -z "${3}" ]]; then
    ...
    TAGS=( $(gcloud container images list-tags gcr.io/devenv-215523/factor-service --limit=10 --format="get(tags)") )
...
kubectl rollout status deployment/factor-service-deployment
