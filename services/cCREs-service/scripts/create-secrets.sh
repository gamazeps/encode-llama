...
# Create ccre-db-credentials secret if it does not exist
if ! kubectl get secret ccre-db-credentials &>/dev/null ; then
    echo "Creating ccres-db-credentials secret..."
    DB_USERNAME=$(jq -r ".ccres_db_username" /tmp/devops/staging-db-credentials/ccres-db.json)
    DB_PASSWORD=$(jq -r ".ccres_db_password" /tmp/devops/staging-db-credentials/ccres-db.json)
    kubectl create secret generic ccre-db-credentials \
        --from-literal=username=$DB_USERNAME \
        --from-literal=password=$DB_PASSWORD
else
    echo "Secret ccres-db-credentials already found. Skipping creation..."
fi
