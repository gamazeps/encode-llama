...
if [[ ! -z "${3}" ]]; then
    ...
    TAGS=( $(gcloud container images list-tags gcr.io/devenv-215523/ccres-service --limit=10 --format="get(tags)") )
...
kubectl rollout status deployment/encode-ccre-service-deployment
