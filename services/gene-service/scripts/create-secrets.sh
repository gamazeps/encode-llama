...
if ! kubectl get secret genes-db-credentials &>/dev/null ; then
    echo "Creating genes-db-credentials secret..."
    ...
    kubectl create secret generic genes-db-credentials \
        --from-literal=username=$DB_USERNAME \
        --from-literal=password=$DB_PASSWORD
else
    echo "Secret genes-db-credentials already found. Skipping creation..."
fi
